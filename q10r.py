...
def _get_submissions(slug):
    ...
    for subm in filter(lambda s: s.endswith('.json'), dirlist):
        with open(os.path.join(sdir, subm), 'r') as f:
            data = json.load(f)
            data = dict(((int(i), [tuple(v) for v in vs]) for i, vs in data.items()))
...
def questionnaire(slug):
    ...
    if request.method == 'GET':
        ...
        for i, q in enumerate(data['questions']):
            ...
            empty_other = (bool(other_opt) and any(True for v, o in s if v == other_opt and not o))
            ...
            if values:
                ...
                q['values'] = values
            if other_opt:
                other_vals = list(filter(None, (o for v, o in s if v == other_opt)))
                if other_vals:
                    q['other_value'] = other_vals[0]
            
            error = error or empty_required or empty_other
